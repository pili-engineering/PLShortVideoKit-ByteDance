...
- (void)setupShortVideoRecorder {
    ...
    self.videoConfiguration.videoOrientation = UIInterfaceOrientationPortrait;
...
//    self.videoConfiguration.videoHardwareType = PLSVideoHardwareTypeHEVC;
    ...
    effectConfiguration.licenseFilePath = [NSString pathWithComponents:@[rootPath, @"LicenseBag.bundle", @"qiniu_20210310_20221011_com.qbox.PLShortVideoKit.ByteDance.Demo_3.9.0.licbag"]];
    ...
    self.shortVideoRecorder = [[PLShortVideoRecorder alloc] initWithVideoConfiguration:self.videoConfiguration audioConfiguration:self.audioConfiguration captureEnabled:YES];
    ...
    [self.shortVideoRecorder setBeautifyModeOn:NO]; // 默认关闭美颜
        ...
        [self.shortVideoRecorder setDeviceOrientationBlock:^(PLSPreviewOrientation deviceOrientation){
            switch (deviceOrientation) {
                ...
                case PLSPreviewOrientationPortraitUpsideDown:
                    self.deviceOrientation = AVCaptureVideoOrientationPortraitUpsideDown;
                ...
                case PLSPreviewOrientationLandscapeLeft:
                    self.deviceOrientation = AVCaptureVideoOrientationLandscapeLeft;
...
- (BEModernEffectPickerView *)effectListView {
    if (!_effectListView) {
        _effectListView = [[BEModernEffectPickerView alloc] initWithFrame:(CGRect)CGRectMake(0, self.view.frame.size.height - 220, self.view.frame.size.width, 220)];
    }
...
- (void)setupBaseToolboxView {
    ...
    UIView *topToolView = [[UIView alloc] initWithFrame:CGRectMake(10, top + 10, 176, 35)];
    ...
    UIView *leftToolView = [[UIView alloc] initWithFrame:CGRectMake(10, top + 60, 72, 228)];
    ...
    beautyFaceButton.frame = CGRectMake(0, 40 * index, 72, 28);
    ...
    self.filterButton.frame = CGRectMake(0, 40 * index, 72, 28);
    ...
    self.monitorButton.frame = CGRectMake(0, 40 * index, 72, 28);
    ...
    self.frameRateButton.frame = CGRectMake(0, 40 * index, 72, 28);
    ...
    self.squareRecordButton.frame = CGRectMake(0, 40 * index, 72, 28);
    ...
    self.filePathButton.frame = CGRectMake(0, 40 * index, 72, 28);
...
- (void)setupRightButtonView {
    ...
    self.rightScrollView.frame = CGRectMake(self.view.bounds.size.width - 60, top, 60, rc.origin.y);
    ...
    [self.effectButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [self.effectButton setTitleColor:PLS_RGBCOLOR(65, 154, 208) forState:UIControlStateSelected];
    [self.effectButton addTarget:self action:@selector(effectButtonDidClick:) forControlEvents:UIControlEventTouchUpInside];
    [self.rightScrollView addSubview:self.effectButton];
    ...
    [self.stickerButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [self.stickerButton setTitleColor:PLS_RGBCOLOR(65, 154, 208) forState:UIControlStateSelected];
    [self.stickerButton addTarget:self action:@selector(stickerButtonDidClick:) forControlEvents:UIControlEventTouchUpInside];
    [self.rightScrollView addSubview:self.stickerButton];
...
#pragma mark - effect related delegate
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    if (touch.view == self.shortVideoRecorder.previewView) {
        return YES;
    }
    return NO;
}
